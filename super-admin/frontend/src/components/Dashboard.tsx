...
const Dashboard: React.FC<DashboardProps> = ({ user, onLogout }) => {
  ...
  const loadGlobalStats = async () => {
    try {
      ...
      if (statsResponse.success) {
        ...
        throw new Error(statsResponse.error || 'Error cargando estadísticas');
      }
    ...
    events: { perSecond: 0, total: 0, totalAlerts: 0, hour: 0 },
  ...
  const loadDashboardData = async () => {
    try {
      ...
      setIsLoading(false);
      // Cargar estadísticas después de cargar empresas
      if (!error) {
    ...
    try {
      ...
      if (typeof apiService.syncCompaniesWithWazuh === 'function') {
        ...
        if (response.success) {
          ...
          setSyncMessage('❌ Error en la sincronización: ' + (response.error || 'Error desconocido'));
        }
      } else {
        setSyncMessage('⚠️ Función de sincronización no disponible');
        console.warn('syncCompaniesWithWazuh no está disponible en apiService');
      }
  ...
  return (
    <div className="min-h-screen bg-slate-900">
      ...
      <div className="p-6">
          ...
          <div className={`border rounded-lg p-4 mb-6 ${
            ...
            <div className="flex items-center">
              <div className={`h-5 w-5 mr-3 ${
                syncMessage.includes('✅') ? 'text-green-400' : 'text-yellow-400'
              }`}>
              ...
              <div>
                ...
                <p className={`text-sm mt-1 ${
                  syncMessage.includes('✅') ? 'text-green-300' : 'text-yellow-300'
                }`}>
                  {syncMessage}
                </p>
              </div>
        ...
        {/* Métricas Generales - Grid de 4 columnas con datos reales */}
            ...
            <div>
              ...
              {globalStats && globalStats.vulnerabilities && globalStats.vulnerabilities.critical > 0 && (
                <div className="text-xs text-red-400 mt-1">
                  {globalStats.vulnerabilities.critical} críticas
                </div>
              )}
            </div>
          </div>
