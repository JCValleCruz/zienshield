...
// ACTUALIZADA: Interfaces para estadísticas globales con eventos reales de Wazuh
export interface GlobalStats {
  ...
  events: {  // NUEVO: Eventos reales de Wazuh
    perSecond: number;
    total: number;
    totalAlerts: number;
    hour: number;
...
class ApiService {
  ...
  // Método para obtener estadísticas globales (incluye eventos reales de Wazuh)
  async getGlobalStats(): Promise<ApiResponse<GlobalStats>> {
    return this.fetchApi('/stats');
  }

  // NUEVO: Método específico para obtener métricas del sistema (para useSystemMetrics)
  async getSystemMetrics(): Promise<ApiResponse<GlobalStats>> {
    return this.fetchApi('/stats');
  }
